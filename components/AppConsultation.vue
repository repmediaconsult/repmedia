<script setup lang="ts"></script>

<template>
    <section class="py-[56px] md:py-[80px] my-[20px] lg:my-[80px]">
        <div class="app-container flex flex-col gap-5">
            <p class="section-title">consultation</p>
            <div class="flex justify-between flex-wrap gap-8">
                <div class="space-y-5 lg:max-w-[510px] xl:max-w-[695px]">
                    <h4 class="section-header">Wanna talk to the “bus”?</h4>
                    <div class="space-y-5 text-[#5A5A5A] paragraph">
                        <p>
                            But I must explain to you how all this mistaken idea of denouncing pleasure and praising pain was born and I will give you a
                            complete account of the system
                        </p>
                        <p>
                            Are you confused about your status in life or career and would like to get in touch with our consultant or carer advisor, book a
                            free 20minutes call here , afterwards is $50 per hour.
                        </p>
                        <p>Gibberish fr</p>
                    </div>
                    <button class="bg-[#000] text-white py-[10px] px-6 rounded-[24px] text-xl -tracking-[4%]">Book Consultation</button>
                </div>
                <img src="/images/consultation.png" alt="consultation" class="h-auto max-w-full w-auto block mx-auto" />
            </div>
        </div>
    </section>
</template>

<style scoped></style>
